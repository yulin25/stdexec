...
namespace experimental::execution
{
  ...
  namespace detail::enter_scopes
  {
    ...
    struct t
    {
      ...
      constexpr ::exec::enter_scope_sender auto operator()(Senders&&... senders) const
        noexcept((std::is_nothrow_constructible_v<std::remove_cvref_t<Senders>, Senders> && ...))
      {
        return ::STDEXEC::__make_sexpr<t>(::STDEXEC::__(), (Senders&&) senders...);
      }
    ...
    class enter_scope_sender_state
    {
      ...
      using exit_scope_sender_type_ = ::exec::exit_scope_sender_of_t<Sender, env_type_>;
      ...
      using rollback_operation_state_type_ =
        ::STDEXEC::connect_result_t<exit_scope_sender_type_, RollbackReceiver>;
      std::variant<operation_state_type_, exit_scope_sender_type_, rollback_operation_state_type_>
                            storage_;
     ...
     public:
      constexpr explicit enter_scope_sender_state(Sender&& sender, Receiver r)
        ...
        : storage_(std::in_place_type<operation_state_type_>,
                   ::exec::elide(
                     [&]() noexcept(nothrow_constructible_)
                     {
                       return ::STDEXEC::connect((Sender&&) sender,
                                                 receiver_type_{{(Receiver&&) r}, *this});
                     }))
      {}
      constexpr void start() & noexcept
      {
        auto const ptr = std::get_if<operation_state_type_>(&storage_);
        STDEXEC_ASSERT(ptr);
        ::STDEXEC::start(*ptr);
      }
      [[nodiscard]]
      ...
      {
        auto const ptr = std::get_if<exit_scope_sender_type_>(&storage_);
        if (!ptr)
        {
          return false;
        }
        auto sender = std::move(*ptr);
        storage_.template emplace<rollback_operation_state_type_>(::exec::elide(
          [&]() noexcept { return ::STDEXEC::connect(std::move(sender), std::move(r)); }));
        return true;
      }
      constexpr void start_rollback() & noexcept
      {
        if (auto const ptr = std::get_if<rollback_operation_state_type_>(&storage_); ptr)
        {
          ::STDEXEC::start(*ptr);
        }
      }
    ...
    using exit_sender_t = decltype(::STDEXEC::when_all(std::declval<Senders>()...));
    ...
    struct storage_for_completion_signatures
    {
      template <typename... Senders>
      using __f =
        ::exec::storage_for_completion_signatures<::STDEXEC::transform_completion_signatures<
          ::STDEXEC::__concat_completion_signatures_t<
            ::STDEXEC::completion_signatures_of_t<Senders, Env...>...>,
    ...
    class state
    {
      ...
      struct receiver_base_
      {
        ...
        env_type_ get_env() const noexcept;
        state&    self_;
      };
      ...
      template <typename S>
      using state_type_ = enter_scope_sender_state<S, receiver_type_, rollback_receiver_type_>;
      Receiver                            r_;
      std::tuple<state_type_<Senders>...> states_;
      //  TODO: There's no need for these two members if no child can fail or stop
      typename storage_for_completion_signatures<::STDEXEC::env_of_t<Receiver>>::template __f<
        Senders...>
                               storage_;
        ...
        std::apply(
          [&](auto&... states) noexcept
          {
            if (stored_.load(std::memory_order_relaxed))
            {
              ...
              auto const impl = [&](auto& state) noexcept
              {
     ...
     public:
      explicit constexpr state(Receiver r, Senders&&... senders)
        noexcept((std::is_nothrow_constructible_v<state_type_<Senders>, Senders, receiver_type_>
                  && ...))
        : r_((Receiver&&) r)
        , states_(::exec::elide(
            [&]() noexcept(
              std::is_nothrow_constructible_v<state_type_<Senders>, Senders, receiver_type_>)
            { return state_type_<Senders>((Senders&&) senders, receiver_type_{{*this}}); })...)
      {}
      constexpr void start() & noexcept
      {
    ...
    template <typename Receiver, typename... Senders>
    inline void state<Receiver, Senders...>::receiver_type_::set_value() && noexcept
    {
      self_.complete_();
    }
    ...
    template <typename Receiver, typename... Senders>
    template <typename... Args>
    inline void state<Receiver, Senders...>::receiver_type_::set_error(Args&&... args) && noexcept
    {
    ...
    template <typename Receiver, typename... Senders>
    template <typename... Args>
    inline void state<Receiver, Senders...>::receiver_type_::set_stopped(Args&&... args) && noexcept
    {
      self_.fail_(::STDEXEC::set_stopped, (Args&&) args...);
    }

    template <typename Receiver, typename... Senders>
    inline void state<Receiver, Senders...>::rollback_receiver_type_::set_value() && noexcept
    {
    ...
    struct completions<Env>
    {
      ...
      using __f = ::STDEXEC::transform_completion_signatures<
        ...
        typename storage_for_completion_signatures<Env>::template __f<
          Children...>::completion_signatures>;
    };
    ...
    struct nothrow_connect
    {
      template <typename... Children>
      using __f =
        std::is_nothrow_constructible<state<Receiver, Children...>, Receiver, Children...>;
    };
    ...
    class impl : public ::STDEXEC::__sexpr_defaults
    {
     ...
     public:
      ...
      static consteval auto __get_completion_signatures()
      {
        ...
        {
          return ::STDEXEC::__throw_compile_time_error<FAILED_TO_FORM_COMPLETION_SIGNATURES,
                                                       ::STDEXEC::_WITH_PRETTY_SENDER_<Sender>>();
        }
      }
      static constexpr auto __connect =
        []<typename Sender, typename Receiver>(Sender&& sender, Receiver r) noexcept(
        ...
        -> ::STDEXEC::__children_of<Sender, connect_result<Receiver>>
      {
        return ::STDEXEC::__apply(
          [&](auto&&, auto&&, auto&&... children) noexcept(
            ::STDEXEC::__children_of<Sender, nothrow_connect<Receiver>>::value)
          {
            return ::STDEXEC::__children_of<Sender, connect_result<Receiver>>((Receiver&&) r,
                                                                              (decltype(children)&&)
                                                                                children...);
          },
          (Sender&&) sender);
