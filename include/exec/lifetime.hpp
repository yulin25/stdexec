...
namespace experimental::execution
{
  ...
  namespace detail::lifetime
  {
    ...
    concept function =
      ::STDEXEC::sender<std::invoke_result_t<F, ::exec::type_of_object_t<Objects>&...>>;

    template <typename SenderFactory, typename... Objects>
    concept sender_factory = ::exec::enter_scope_sender<
      std::invoke_result_t<SenderFactory, ::exec::enter_scope_sender_of_object_t<Objects>...>>;
    ...
    struct t
    {
      ...
      constexpr ::STDEXEC::sender auto operator()(F&& f, Objects&&... objects) const
        noexcept(std::is_nothrow_constructible_v<std::remove_cvref_t<F>, F>
                 && (std::is_nothrow_constructible_v<std::remove_cvref_t<Objects>, Objects> && ...))
      {
        return (*this)(::exec::enter_scopes, (F&&) f, (Objects&&) objects...);
      }
      template <typename SenderFactory, typename F, ::exec::object... Objects>
        requires sender_factory<SenderFactory, Objects...> && function<F, Objects...>
      constexpr ::STDEXEC::sender auto
      operator()(SenderFactory&& sender_factory, F&& f, Objects&&... objects) const
        noexcept(std::is_nothrow_constructible_v<std::remove_cvref_t<SenderFactory>, SenderFactory>
                 && std::is_nothrow_constructible_v<std::remove_cvref_t<F>, F>
                 && (std::is_nothrow_constructible_v<std::remove_cvref_t<Objects>, Objects> && ...))
      {
        return ::STDEXEC::__make_sexpr<t>(
          std::tuple((SenderFactory&&) sender_factory, (F&&) f, (Objects&&) objects...));
      }
    };
    ...
    using storage_for_object_t = storage_for_object<::exec::type_of_object_t<Object>>;

    template <typename... Objects>
    using storage_for_objects_t = std::tuple<storage_for_object_t<Objects>...>;

    template <typename Tuple, typename = std::remove_cvref_t<Tuple>>
    class make_sender;

    template <typename Tuple, typename SenderFactory, typename F, typename... Objects>
    class make_sender<Tuple, std::tuple<SenderFactory, F, Objects...>>
    {
      static constexpr auto
      impl_(::STDEXEC::__copy_cvref_t<Tuple, F>&& f, storage_for_object_t<Objects>&... storage)
        noexcept(std::is_nothrow_constructible_v<F, ::STDEXEC::__copy_cvref_t<Tuple, F>>)
      {
        return [f = (::STDEXEC::__copy_cvref_t<Tuple, F>&&) f, &storage...]() mutable noexcept(
                 std::is_nothrow_invocable_v<F, ::exec::type_of_object_t<Objects>&...>)
        {
          return std::invoke(std::move(f), storage.get_object()...);
        };
      }
     public:
      static constexpr bool nothrow = noexcept(::exec::within(
        std::invoke(std::declval<::STDEXEC::__copy_cvref_t<Tuple, SenderFactory>>(),
                    std::invoke(std::declval<::STDEXEC::__copy_cvref_t<Tuple, Objects>>(),
                                std::declval<::exec::type_of_object_t<Objects>*>())...),
        ::STDEXEC::just()
          | ::exec::invoke(impl_(std::declval<::STDEXEC::__copy_cvref_t<Tuple, F>>(),
                                 std::declval<storage_for_object_t<Objects>&>()...))));
      using storage_type            = storage_for_objects_t<Objects...>;
      static constexpr ::STDEXEC::sender auto
      impl(Tuple&& t, storage_type& storage) noexcept(nothrow)
      {
        return std::apply(
          [&](auto&& sender_factory, auto&& f, auto&&... objects) noexcept(nothrow)
          {
            return std::apply(
              ...
              {
                return ::exec::within(std::invoke((decltype(sender_factory)&&) sender_factory,
                                                  std::invoke((decltype(objects)&&) objects,
      ...
      using sender_type = decltype(impl(std::declval<Tuple>(),
    ...
    class state
    {
      ...
      ::STDEXEC::connect_result_t<typename impl_::sender_type, Receiver> op_;
     public:
      explicit constexpr state(Tuple&& t, Receiver r) noexcept(impl_::nothrow)
        : op_(::STDEXEC::connect(impl_::impl((Tuple&&) t, storage_), (Receiver&&) r))
      {}
     ...
     public:
      ...
      static constexpr auto __get_state =
        []<typename Sender, typename Receiver>(Sender&& sender, Receiver r) noexcept(
          std::is_nothrow_constructible_v<state<::STDEXEC::__data_of<Sender>, Receiver>,
                                          ::STDEXEC::__data_of<Sender>,
                                          Receiver>)
        -> state<::STDEXEC::__data_of<Sender>, Receiver>
      {
        auto&& [_, tuple] = (Sender&&) sender;
        return state<decltype(tuple), Receiver>((decltype(tuple)&&) tuple, (Receiver&&) r);
      };
