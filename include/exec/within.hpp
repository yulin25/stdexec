...
namespace experimental::execution
{
  namespace detail::within
  {
    struct t
    {
      ...
      constexpr ::STDEXEC::sender auto operator()(Scope&& scope, Sender&& sender) const
      ...
      {
        return ::STDEXEC::__make_sexpr<t>(::STDEXEC::__(), (Scope&&) scope, (Sender&&) sender);
      }
    };
    ...
    inline constexpr bool nothrow_connect_sender =
      ::STDEXEC::__nothrow_connectable<Sender, ::STDEXEC::__receiver_archetype<Env>> &&
      //  Because we move the sender out of storage so we can reuse it
      std::is_nothrow_move_constructible_v<Sender>;

    template <typename Sender, typename Env>
    using completion_signatures_of_sender_t = ::STDEXEC::transform_completion_signatures<
    ...
    using completion_signatures_of_scope_t = ::STDEXEC::transform_completion_signatures<
    ...
    using storage_for_completion_signatures_t =
      ::exec::storage_for_completion_signatures<completion_signatures_of_sender_t<Sender, Env>>;
    ...
    struct completions
    {
      ...
      using __f = ::STDEXEC::transform_completion_signatures<
        typename storage_for_completion_signatures_t<std::remove_cvref_t<Sender>,
                                                     Env>::completion_signatures,
        completion_signatures_of_scope_t<Scope, Env>>;
    };
    ...
    class state
    {
      ...
      using exit_scope_sender_type_ = ::exec::exit_scope_sender_of_t<Scope, env_type_>;
      ...
      std::variant<enter_state_type_, state_type_, exit_operation_state_type_> state_;
     ...
     public:
      template <typename S>
      explicit constexpr state(Scope&& scope, S&& sender, Receiver r)
        noexcept(std::is_nothrow_constructible_v<Sender, S> && nothrow_construct_connect_);
      constexpr void start() & noexcept;
    };

    template <typename Scope, typename Sender, typename Receiver>
    inline auto
    state<Scope, Sender, Receiver>::receiver_base_::get_env() const noexcept -> env_type_
    {
      return ::STDEXEC::get_env(self_.r_);
    }

    template <typename Scope, typename Sender, typename Receiver>
    constexpr void state<Scope, Sender, Receiver>::exit_() noexcept
    {
      auto const ptr    = std::get_if<state_type_>(&state_);
      auto       sender = std::move(ptr->s);
      auto&&     op     = state_.template emplace<exit_operation_state_type_>(::exec::elide(
        [&]() noexcept
        { return ::STDEXEC::connect(std::move(sender), exit_receiver_type_{{*this}}); }));
      ::STDEXEC::start(op);
    }

    template <typename Scope, typename Sender, typename Receiver>
    constexpr void state<Scope, Sender, Receiver>::start() & noexcept
    {
      auto const ptr = std::get_if<enter_state_type_>(&state_);
      STDEXEC_ASSERT(ptr);
    ...
    template <typename Scope, typename Sender, typename Receiver>
    template <typename S>
    constexpr state<Scope, Sender, Receiver>::state(Scope&& scope, S&& sender, Receiver r)
      noexcept(std::is_nothrow_constructible_v<Sender, S> && nothrow_construct_connect_)
      : r_((Receiver&&) r)
      , state_(std::in_place_type<enter_state_type_>,
               ::exec::elide(
                 [&]() noexcept(nothrow_construct_connect_)
                 {
                   return enter_state_type_{::STDEXEC::connect((Scope&&) scope,
    ...
    template <typename Scope, typename Sender, typename Receiver>
    inline void state<Scope, Sender, Receiver>::enter_receiver_type_::set_value(
      exit_scope_sender_type_ exit) && noexcept
    {
      ...
      auto const     ptr     = std::get_if<enter_state_type_>(&self.state_);
      STDEXEC_ASSERT(ptr);
      try
      {
        auto   sender = (Sender&&) ptr->s;
        auto&& state  = self.state_.template emplace<state_type_>(::exec::elide(
          [&]() noexcept(nothrow)
          {
            return state_type_{::STDEXEC::connect((Sender&&) sender, receiver_type_{{self}}),
                               std::move(exit)};
          }));
    ...
    template <typename... Args>
    constexpr void
    state<Scope, Sender, Receiver>::receiver_type_::set_value(Args&&... args) && noexcept
    {
    ...
    template <typename... Args>
    constexpr void
    state<Scope, Sender, Receiver>::receiver_type_::set_error(Args&&... args) && noexcept
    {
    ...
    template <typename... Args>
    constexpr void
    state<Scope, Sender, Receiver>::receiver_type_::set_stopped(Args&&... args) && noexcept
    {
