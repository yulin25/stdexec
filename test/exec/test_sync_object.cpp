...
namespace
{
  ...
  class object
  {
   ...
   public:
    int i;
    explicit constexpr object(state& s, int i) noexcept
      : s_(s)
      , i(i)
    {
      ++s_.constructed;
...
//  difficult to address
  ...
  //       bool&          b_;
  //       constexpr void set_value(int const i) && noexcept
  //       {
  //         b_ = i == 5;
  //       }
  //     };
  //     auto sender  = ::exec::lifetime([&](object& o) noexcept { return ::STDEXEC::just(o.i); },
  //                                    ::exec::make_sync_object<object>(std::ref(s), 5));
  //     bool success = false;
  //     auto op      = ::STDEXEC::connect(std::move(sender), receiver{success});
  //     ::STDEXEC::start(op);
  //     return success;
  //   }());
#endif
  ...
  TEST_CASE("Synchronous object may be adapted into asynchronous objects", "[sync_object]")
  {
    ...
    auto  sender = ::exec::lifetime(
      ...
      ::exec::make_sync_object<object>(std::ref(s), 5));
    auto op = ::STDEXEC::connect(std::move(sender), expect_value_receiver(5));
